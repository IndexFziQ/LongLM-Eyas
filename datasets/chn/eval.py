...
def compute(golden_file, pred_file, return_dict=True):
    ...
    overall_score = w[0]*bleu_score['bleu-1'] + w[1]*bleu_score['bleu-2'] + w[2]*dist_score['distinct-3'] +  w[3]*dist_score['distinct-4'] +  w[4]*cover_score['coverage'] +   w[5]*order_score['order']
...
def main():
    ...
    all_score, overall_score = compute(argv[2], argv[1])
    print("所有指标：{}".format(all_score))
    print("加权总得分：{}".format(overall_score))
